...
bool system_hardware_init(
  ...
  // 5. Initialize Lift Hardware
  Serial.println("[5/6] Initializing Lift Hardware...");
  bool dac_ok = false, adc_ok = false;
  lift.checkHardware(dac_ok, adc_ok);
  if (dac_ok) {
    Serial.println("      GP8413 0-10V DAC found (0x" + String(I2C_DAC_ADDR, HEX) + ")");
  } else {
    Serial.println("      ❌ GP8413 0-10V DAC NOT FOUND (0x" + String(I2C_DAC_ADDR, HEX) + ")");
    i2c_error++;
  }
